...
const BotRow = ({ bot, allPairs, isClosed }: { bot: Bot, allPairs: DScore[], isClosed?: boolean }) => {
    ...
    // Find the d_score_entry from the bot data, which may not exist on old mock data
    // The `addBot` action doesn't add this yet, so we'll need to update it later.
    const dScoreEntry = (bot as any).d_score_entry ?? (bot as any).dScoreEntry ?? 'N/A';

    return (
        <TableRow key={bot.id}>
            <TableCell className="font-medium">{bot.pair}</TableCell>
            <TableCell>{(bot as any).strategy || 'DCA Grid'}</TableCell>
            <TableCell>
            ...
            <TableCell>{typeof dScoreEntry === 'number' ? dScoreEntry.toFixed(1) : dScoreEntry}</TableCell>
