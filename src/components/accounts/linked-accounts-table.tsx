...
export default function LinkedAccountsTable({ accounts: initialAccounts }: LinkedAccountsTableProps) {
  ...
  const handleWeightChange = (id: string, weight: number) => {
    // Ensure weight is not negative and has a minimum for very small values
    const newWeight = Math.max(0, weight);
    if (newWeight * 100 < 0.01 && newWeight > 0) {
      // Potentially show a warning, for now just setting state
    }
     setAccounts(prev => prev.map(acc => 
